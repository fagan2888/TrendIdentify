# -*- coding: utf-8 -*-
"""
Created on Thu Dec  5 16:45:07 2019
...
def build():
    # define a LSTM model
    # input_shape(picture_shape, channels)
    # Conv2D(nerves,coreshape,input_shape=3Dtensor )
    model = Sequential()
    model.add(Conv2D(100,(6,6), input_shape = (201,6,3)))
...
def handle(code, sdate, edate):
    ...
    # x should be a 4D tensor for CNN input
    X = np.zeros((201,6,1))
    X[:,0,:] = x
    y = df.close
